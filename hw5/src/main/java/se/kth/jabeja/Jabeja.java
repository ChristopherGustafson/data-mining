...
public class Jabeja {
  ...
  private float tMin = 0.00001F;
  private boolean resetting = false;
  private int resetRound = 400;
  ...
  public Jabeja(HashMap<Integer, Node> graph, Config config) {
    ...
    // If using simulated annealing, temperature should be initialized to 1
    if(annealing) this.T = 1;
  }
  ...
  private void saCoolDown(){
    ...
    if(annealing) {
      ...
      if (T < tMin)
        T = tMin;
      if(resetting && (round % resetRound) == 0)
        T = 1;
    }
    else {
      if (T > 1)
  ...
  private double acceptanceProbability(double newEnergy, double oldEnergy) {
  ...
  public Node findPartner(int nodeId, Integer[] nodes){
    ...
    for(Integer qID: nodes) {
      ...
      // No need to try to swap if colors are the same
      if(checkColors && nodeP.getColor() == nodeQ.getColor()) continue;
      // old = dpp + dqq
      ...
      if(annealing){
          double acceptance = acceptanceProbability(newEnergy, oldEnergy);
          double random = Math.random();
          if(newEnergy != oldEnergy && random < acceptance && acceptance > highestBenefit) {
            bestPartner = nodeQ;
